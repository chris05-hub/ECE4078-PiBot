...
class IMDB(Dataset):
    def __init__(self, root_dir, mode):
        ...
        self.transform = transforms.Compose(
            [transforms.ColorJitter(brightness=0.4, contrast=0.2, saturation=0.4, hue=0.05),
            transforms.ToTensor(),
    ...
    def __getitem__(self, idx):
        ...
        reduction = 2
        label = label.resize((int(width/reduction), int(height/reduction)), Image.NEAREST)
...
def imdb_loader(args):
    train_loader = DataLoader(dataset=IMDB(args.dataset_dir, mode='train'), batch_size=args.batch_size, shuffle=True)
    eval_loader = DataLoader(dataset=IMDB(args.dataset_dir, mode='eval'), batch_size=args.batch_size, shuffle=False)
