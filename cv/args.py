...
file_path = 'object_list.csv'
if not os.path.exists(file_path):
    file_path = os.path.join('..', file_path)
with open(file_path, 'r') as file:
    reader = csv.DictReader(file)
    data = list(reader)
parser.add_argument("--n_classes", metavar='', type=int, default=len(data))
...
parser.add_argument("--scheduler_step", metavar='', type=int, default=5, help='the learning rate is reduced to <?>')
parser.add_argument("--scheduler_gamma", metavar='', type=float, default=0.5, help='the learning rate is reduced to <?> every <?> epochs')

# Trainer Configuration, no changes neccessary
parser.add_argument("--model_dir", metavar='', type=str, default="model", help="folder to save the experiment")
parser.add_argument("--load_best", metavar='', type=int, default=0, help=' load the best checkpoint')
parser.add_argument("--log_freq", metavar='', type=int, default=20, help=" record a training log every <?> mini-batches")
