...
def main(args):
    ...
    for image_name in all_images:
        ...
        label_path = os.path.join('dataset', 'labels', image_name[:-4]+'_label.png')
    ...
    if 0 < train_ratio < 1:
        ...
        generate_binary_file(train_catalog, os.path.join(binary_dir, 'train.hdf5'))
        generate_binary_file(eval_catalog, os.path.join(binary_dir,'eval.hdf5'))
    else:
...
if __name__ == '__main__':
    generator_parser = argparse.ArgumentParser(description='Split dataset for trainig and evaluation')
    generator_parser.add_argument('--output_dir', type=str, default='')
    generator_parser.add_argument('--training_ratio' ,  type=float, default=0.8, help='Ratio of trainig over evaluation')
