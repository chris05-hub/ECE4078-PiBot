...
def calibrateScale():
    ...
    wheel_speed_range = [[0.3, 0.3], [0.35, 0.35], [0.4, 0.4]]
    delta_times = []
    ...
    for delta_time, wheel_speed in zip(delta_times, wheel_speed_range):
        # actual speed = raw speed * scale = distance / time
        raw_speed = wheel_speed[0]
        actual_speed = 1.0/delta_time
        scale_sum += actual_speed/raw_speed
...
def calibrateBaseline(scale):
    ...
    baseline_sum = 0
    for delta_time, wheel_speed in zip(delta_times, wheel_speed_range):
        raw_speed = abs(wheel_speed[0])
        # angular velocity = 2pi/time
        # speed = radius * angular velocity
        # baseline = 2*radius
        # speed = baseline/2 * angular velocity
        # baseline = (2*speed)/angular velocity
        #          = (2*speed*time)/(2pi)
        baseline_sum += (2 * raw_speed * scale * delta_time) / (2 * np.pi)
        # pass # TODO: replace with your code to compute the baseline parameter using scale, wheel_speed, and delta_time
    baseline = baseline_sum / num
...
if __name__ == "__main__":
    ...
    botconnect.set_pid(use_pid=1, kp=2.5, ki=0.2, kd=0) # TODO: replace with your best constants
