...
class Trainer:
    ...
    def fit(self, model, train_loader, eval_loader):
        ...
        model, optimiser, lr_scheduler = self.load_ckpt(model, optimiser, lr_scheduler)
        if self.last_epoch == -1:
            self.init_log(model)
        for epoch_idx in range(self.last_epoch + 1, self.args.epochs):
            self.current_epoch = epoch_idx
            clock = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            self.log(cmd_printer.divider(text=f'Epoch {epoch_idx} | {clock}', show=False))
            ...
            # output to log
            self.log(f'\n=> Training Loss: {avg_train_loss:.4f}, Evaluation Loss {loss_eval:.4f}')
            self.log('\n')
            # output to terminal
            print(f'\n=> Training Loss: {avg_train_loss:.4f} , Evaluation Loss {loss_eval:.4f}')
    ...
    def load_ckpt(self, model, optimiser=None, lr_scheduler=None):
        ckpt_suffix = '.best.pt' if self.args.load_best else '.pt'
    ...
    def save_ckpt(self, model, optimiser, lr_scheduler=None):
        ...
        if self.loss_reduction > 0:
            ...
            if self.current_epoch > 0:
                print(f'=> Best Model Updated, {self.loss_reduction:.3f} Eval Loss Reduction\n')
            else:
                print('\n')
        else:
            print('=> Model Saved\n')
